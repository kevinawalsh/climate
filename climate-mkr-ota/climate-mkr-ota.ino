...
#define OTA_UPDATE_URL ("https://assembler.kwalsh.org/climate/update-from-v" VERSION ".bin")
...
bool rate_limit_expired(unsigned long *last_update, unsigned long interval) {
  ...
  if (*last_update == 0 || (time_now - *last_update) >= interval) {
...
char urlbuf[120];
bool http_split_url(char *url, char **serverp, char **pathp, int *portp, bool *useSSLp) {
...
bool http_open_method(WiFiClient *client, char *method, char *url, int bodylen) {
  ...
  int port;
  bool useSSL;
  if (!http_split_url(url, &server, &path, &port, &useSSL))
      return false;
  Serial.print(method);
  Serial.print(" ");
  Serial.println(url);
...
bool check_for_ota_updates() {
  LOG("Check for OTA update: "); LOG(OTA_UPDATE_URL); LOG("\n");
  char *server, *path;
  int port;
  bool useSSL;
  if (!http_split_url(OTA_UPDATE_URL, &server, &path, &port, &useSSL))
      return false;
  LOGF("SERVER [%s]\n", server);
  LOGF("PORT [%d]\n", port);
  LOGF("SSL [%s]\n", useSSL ? "yes" : "no");
  ...
  LOG("Update status code: "); LOG(statusCode); LOG("\n");
  if (statusCode != 200) {
    client->stop();
    return false;
  }
...
void loop() {
  ...
  if (rate_limit_expired(&ota_timestamp, 5000)) {
